...
/*原来是将配置信息写在xml文件中，现在采用java的显示配置，因此通常将排；位置信息的类以config结尾，我们需要在类上使用 @Configuration,
以明确的指出该类是bean配置的信息源
并且 Spring 对标注Configuration 的类有如下要求：
...
@Bean 的作用等价于 XML 配置中的 标签。示例如下：  http://blog.csdn.net/zly9923218/article/details/52150907
	@Configuration  
...
@Configuration	
@ComponentScan  //(basePackageClasses={MusicCD.class, InstantiationTracingBeanPostProcessor.class})	这么配置也是行的	
